  ...
  constructor(options: UploadifyOptions = {}) {
    ...
    if (!this.options.storage) {
      if (this.options.dest) {
        this.options.storage = new DiskStorage({
  ...
  private async handleRequest(
    ...
    const parser = new MultipartParser(
      ...
      (part: Part) => {
        ...
        // Check limits
        if (
          this.options.limits?.files &&
          fileCount >= this.options.limits.files
        ) {
          part.stream.resume();
        ...
        if (this.options.fileFilter) {
          this.options.fileFilter(req, file, (err, accept) => {
            if (err || !accept) {
              part.stream.resume();
              if (err)
                this.addNotification(req, "FILE_FILTER_ERROR", err.message);
              return;
            }
      ...
      (name, value) => {
        // Handle Field
        if (
          this.options.limits?.fieldNameSize &&
          name.length > this.options.limits.fieldNameSize
        ) {
        ...
        if (
          ...
          value.length > this.options.limits.fieldSize
        ) {
